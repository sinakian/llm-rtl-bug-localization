...
agent: dataset
	$(PYTHON) run_agent_eval.py --model qwen2.5-coder:latest --seed 0 --out results/predictions_agent.json
...
baselines: agent
	...
	$(PYTHON) random_baseline.py --agent-predictions results/predictions_agent.json --seeds 100

# Score everything
eval: baselines
	$(PYTHON) evaluate.py results/predictions_agent.json
